...
async function createSha256Hash(bytes: Uint8Array): Promise<string> {
  if (!globalThis.crypto?.subtle) {
    throw new Error("Web Crypto subtle.digest is unavailable in this runtime.");
  }
...
function createRuntimeArmourContract(): LedgerArmourContract {
  return {
    async encrypt(input) {
      await initArmour();
      ...
      return {
        data: input.encoding === "armor" ? textDecoder.decode(ciphertext) : encodeBase64Url(ciphertext),
        payloadHash: await createSha256Hash(ciphertext),
      };
    },
    async decrypt(input) {
      await initArmour();
      ...
      try {
        const plaintext = await decryptWithIdentity({
          ...
          data: new Uint8Array(toCiphertextBytes(input.payload)),
        });
        return JSON.parse(textDecoder.decode(plaintext));
      } catch {
        return {
          __runtimeEncryptedPayload: {
            ...input.payload,
          },
        };
      }
...
function readConflictInfo(entry: LedgerEntryRecord): StagedEntryConflictInfo | null {
  ...
  const aggregateId =
    readText(runtimeConflict?.aggregateId) ?? readText(payload.aggregateId) ?? undefined;
  const actorIdentityKey =
    readText(runtimeConflict?.actorIdentityKey) ??
    readText(payload.actorIdentityKey) ??
    readText(payload.updatedBy) ??
    readText(payload.createdBy) ??
    undefined;
  const permissionId =
    readText(runtimeConflict?.permissionId) ??
    readText(payload.permissionId) ??
    readText(payload.audienceId) ??
    undefined;
...
function conflictsForEntries(input: {
  ...
  for (const stagedInfo of stagedInfos) {
    if (stagedInfo.permissionId && stagedInfo.actorIdentityKey) {
    ...
    for (const remoteInfo of remoteInfos) {
      ...
      conflicts.push({
        ...
        aggregateId: stagedInfo.aggregateId,
        message:
          "Remote changes modified the same aggregate field from the same base revision as a staged change.",
      });
    }
  }
...
export async function createApp(input: CreateAppInput): Promise<AppRuntime> {
  ...
  const runtimeArmour = createRuntimeArmourContract();

  async function createConcord(): Promise<ConcordApp> {
    return await createConcordApp({
      identity: input.identity,
      storage: resolvedStorageAdapter,
      armour: runtimeArmour,
      plugins: input.plugins.map((plugin) => plugin.plugin),
    });
  }

  let concord = await createConcord();
  const concordListeners = new Set<(state: Readonly<ReturnType<ConcordApp["getState"]>>) => void>();
  let concordUnsubscribe: (() => void) | null = null;

  function notifyConcordListeners(nextState: Readonly<ReturnType<ConcordApp["getState"]>>): void {
    for (const listener of concordListeners) {
      listener(nextState);
    }
  }

  function bindConcordSubscription(): void {
    concordUnsubscribe?.();
    concordUnsubscribe = concord.subscribe((nextState) => {
      notifyConcordListeners(nextState);
    });
  }

  async function recreateConcord(): Promise<void> {
    concordUnsubscribe?.();
    concordUnsubscribe = null;
    await concord.destroy();
    concord = await createConcord();
    bindConcordSubscription();
  }

  bindConcordSubscription();

  const localProvider = createLocalRuntimeStorageProvider({ storage: resolvedStorageAdapter });
  ...
  async function getStorageSnapshot() {
    return await resolvedStorageAdapter.load();
  }

  async function reopenFromActiveStorageRef(): Promise<RuntimeReplayResult> {
    const provider = storageProviders.get(activeStorageRef.providerId);
    if (!provider) {
      throw new Error(`Storage provider '${activeStorageRef.providerId}' is unavailable.`);
    }

    if (!provider.capabilities.supportsLoad) {
      throw new Error(`Storage provider '${provider.id}' does not support load.`);
    }

    const snapshot = await provider.load(activeStorageRef);
    await resolvedStorageAdapter.save({
      container: snapshot.container,
      staged: [],
    });

    await recreateConcord();
    return await replayPipeline({
      mode: "load",
      replay: { reason: "manual" },
    });
  }

  async function clearPersistedStagedEntries(): Promise<void> {
    ...
    if (provider.capabilities.supportsPull) {
      ...
      try {
        ...
        if (localSnapshotBeforePull) {
          await resolvedStorageAdapter.save(localSnapshotBeforePull);
  ...
  return {
    ...
    reopen() {
      return reopenFromActiveStorageRef();
    },
    ...
    async discard() {
      await clearPersistedStagedEntries();
      await recreateConcord();
      await concord.load();
    },
    replay(options) {
    ...
    async discardWithReplay() {
      await clearPersistedStagedEntries();
      await recreateConcord();
      return await replayPipeline({ mode: "load", replay: { reason: "discard" } });
    },
    ...
    select(pluginId, selectorId, ...args) {
      ...
      return selector(concord.getReplayState(pluginId), ...args) as any;
    },
    subscribe(listener) {
      concordListeners.add(listener);
      return () => {
        concordListeners.delete(listener);
      };
    },
    async destroy() {
      concordUnsubscribe?.();
      concordUnsubscribe = null;
      concordListeners.clear();
      await concord.destroy();
    },
    getActiveStorageRef() {
